  ...
  return (
    <div className="h-full w-full overflow-auto flex items-center justify-center">
      ...
      <motion.div
        ...
        animate={{
          background: [
            `radial-gradient(ellipse at 40% 40%, ${palette.primary}08 0%, transparent 60%), linear-gradient(180deg, #f8fafc, white, #f8fafc)`,
            `radial-gradient(ellipse at 60% 60%, ${palette.secondary}08 0%, transparent 60%), linear-gradient(180deg, #f8fafc, white, #f8fafc)`,
            `radial-gradient(ellipse at 40% 40%, ${palette.primary}08 0%, transparent 60%), linear-gradient(180deg, #f8fafc, white, #f8fafc)`,
          ],
        }}
      ...
      <motion.div
        ...
        transition={{ duration: 0.8, delay: baseDelay, ease: [0.22, 1, 0.36, 1] as const }}
      >
        {/* Glass card */}
        <div
          className="rounded-3xl border border-white/60 bg-white/70 backdrop-blur-xl p-8 md:p-10 shadow-[0_8px_40px_rgba(0,0,0,0.06)]"
        >
          {/* Accent glow at top */}
          <div
            ...
            {links.map((link, i) => {
              ...
              return (
                <motion.a
                  ...
                  className="flex items-center gap-4 px-5 py-3.5 rounded-xl border border-gray-100 bg-white/60 hover:bg-white hover:shadow-md transition-all duration-300 group"
                  initial={{ opacity: 0, x: -20 }}
                  animate={{ opacity: 1, x: 0 }}
                  transition={{ delay: baseDelay + 0.4 + 0.1 * i, duration: 0.5 }}
                  whileHover={{ x: 4 }}
                >
                  <div
